...
class TraxInfoParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == 'img' or tag == 'div':
            ...
            for attr in attrs:
                ...
                elif attr[0]=='id' and attr[1] == 'message':
                    self.gather_error = True
        elif tag == 'span':
            for attr in attrs:
                if attr[0] == 'id' and attr[1] in info.keys():
                    self.gather_info = info[attr[1]]
                elif attr[0] == 'style' and attr[1] == 'font-family:Verdana; color:rgb(0,0,0);':
                    self.gather_data = True
                    
        elif self.gather_data and tag == 'br':
                self.notes += '<br>'
    ...
    def handle_endtag(self, tag):
        ...
        #print("\t Ending %s tag" % self.name)
        self.maybe_save_tag()
        ##give up on tag
        self.name = self.onclick = ''
        if self.gather_data and tag == 'span':
            self.gather_data = False
    ...
    def handle_data(self, data):
        if self.gather_info:
            self.info[self.gather_info] = data
            self.gather_info = False
        if self.gather_data:
            self.notes += data
        if self.gather_error:
